...
serve(async (req) => {
  ...
  try {
    ...
    if (!authHeader?.startsWith('Bearer ')) {
      console.log('Microsoft OAuth start: Missing or invalid auth header');
      return new Response(
    ...
    if (userError || !user) {
      console.log('Microsoft OAuth start: User authentication failed', { error: userError?.message });
      return new Response(
        JSON.stringify({ error: 'Unauthorized' }),
        { status: 401, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    ...
    if (!clientId) {
      console.error('Microsoft OAuth start: MICROSOFT_CLIENT_ID not configured');
      return new Response(
    ...
    if (!isAllowedOrigin(origin)) {
      console.log('Microsoft OAuth start: Origin rejected', { origin });
      return new Response(
    ...
    console.log('Microsoft OAuth start: Returning auth URL successfully');
